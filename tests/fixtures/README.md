...
| `iperf-overlay.tsv` | the same run through `iperf_orchestrator`'s own `export-overlay`, which writes this format directly; the suite asserts it agrees with `dcimport --iperf` sample for sample |
